...
public class HuaweiRemoteAccess extends CommutatorCredentials implements AbstractRemoteAccess {
    ...
    public List<PortInfo> getPorts() {
        ...
        String page = portsData;
        Pattern lastPagePattern = Pattern.compile("<[^>]+>");
        Instant start = Instant.now();
        while (true) {
            if(Duration.between(start, Instant.now()).getSeconds() > 30) {
                throw new ParsingException("Таймаут парсинга интерфейсов Huawei, не достигнута последняя страница интерфейсов.");
            }
            if (lastPagePattern.matcher(page).find()) {
                break;
            }
            page = telnetParser.send(" ");
            portsData += page;
        }
        ...
        while (true){
            if(Duration.between(start, Instant.now()).getSeconds() > 30)
                throw new ParsingException("Таймаут парсинга MAC адресов Huawei, не достигнута последняя страница MAC адресов.");
            if (lastPagePattern.matcher(page).find())
                break;
            page = telnetParser.send(" ");
            fdbData += page;
