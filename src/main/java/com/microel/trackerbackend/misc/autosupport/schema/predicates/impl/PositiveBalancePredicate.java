...
import static com.microel.trackerbackend.misc.autosupport.schema.predicates.PredicateType.POSITIVE_BALANCE;

public class PositiveBalancePredicate implements IPredicate {
    @Override
    ...
    public Boolean evaluate(AutoSupportContext context, Map<String, String> params, Long userId) {
        try {
            return context.getBillingController().getUserInfo(params.get("username")).getIbase().getMoney() >= 0;
        } catch (Exception e) {
